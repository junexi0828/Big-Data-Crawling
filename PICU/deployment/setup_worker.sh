...
# requirements 파일 전송 (base.txt와 worker.txt)
rsync -avz \
...
ssh "$WORKER_USER@$WORKER_IP" << EOF
    ...
    # requirements 파일 설치
    if [ -f "$PROJECT_DIR/worker.txt" ]; then
        pip install -r $PROJECT_DIR/worker.txt
        echo "✓ Installed from worker.txt"
    else
        echo "✗ worker.txt not found in $PROJECT_DIR"
        exit 1
    fi
EOF
...
if [ -d "$HADOOP_ROOT" ]; then
    ...
    rsync -avz --delete \
        ...
        --exclude='*.log' \
        "$HADOOP_ROOT/" \
        "$WORKER_USER@$WORKER_IP:$HADOOP_INSTALL_DIR/"

    echo -e "${GREEN}✅ Hadoop 바이너리 배포 완료${NC}"

    # Hadoop 설정 파일 생성 (DataNode용)
    ssh "$WORKER_USER@$WORKER_IP" << EOF
        mkdir -p $HADOOP_INSTALL_DIR/etc/hadoop

        # core-site.xml (NameNode 주소)
