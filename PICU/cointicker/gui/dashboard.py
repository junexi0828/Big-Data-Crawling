...
class DashboardApp:
    ...
    def _create_menu(self):
        ...
        file_menu.add_command(
            label="새로고침", command=self.refresh_all, accelerator="F5"
        )
    ...
    def _create_cluster_tab(self, parent):
        ...
        ttk.Button(top_frame, text="새로고침", command=self.refresh_cluster).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(top_frame, text="HDFS 상태", command=self.show_hdfs_status).pack(
            side=tk.LEFT, padx=5
        )
        ...
        scrollbar = ttk.Scrollbar(
            parent, orient=tk.VERTICAL, command=self.cluster_tree.yview
        )
    ...
    def _create_tier2_tab(self, parent):
        ...
        ttk.Button(top_frame, text="새로고침", command=self.refresh_tier2).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(
            top_frame, text="인사이트 생성", command=self.generate_insights
        ...
        self.tier2_status_text = scrolledtext.ScrolledText(
            status_frame, height=15, wrap=tk.WORD
        )
        ...
        self.tier2_summary_text = scrolledtext.ScrolledText(
            summary_frame, height=10, wrap=tk.WORD
        )
    ...
    def _create_control_tab(self, parent):
        ...
        ttk.Button(button_frame, text="Spider 시작", command=self.start_spider).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(button_frame, text="Spider 중지", command=self.stop_spider).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(
            button_frame, text="파이프라인 재시작", command=self.restart_pipeline
        ).pack(side=tk.LEFT, padx=5)
        ...
        self.control_log_text = scrolledtext.ScrolledText(
            log_frame, height=20, wrap=tk.WORD
        )
    ...
    def _create_config_tab(self, parent):
        ...
        self.config_text = scrolledtext.ScrolledText(
            cluster_config_frame, height=15, wrap=tk.WORD
        )
        ...
        ttk.Entry(tier2_inner, textvariable=self.tier2_url_var, width=40).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(tier2_inner, text="적용", command=self.update_tier2_url).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def refresh_cluster(self):
        ...
        def _refresh():
            try:
                ...
                self.root.after(
                    0, lambda: self.status_var.set("클러스터 상태 업데이트 완료")
                )
    ...
    def refresh_tier2(self):
        ...
        def _refresh():
            try:
                ...
                self.root.after(
                    0, lambda: self.status_var.set("Tier2 서버 상태 업데이트 완료")
                )
            except Exception as e:
    ...
    def _update_tier2_status(self, status: Dict, summary: Dict):
        ...
        self.tier2_status_text.insert(
            tk.END, json.dumps(status, indent=2, ensure_ascii=False)
        )

        self.tier2_summary_text.delete(1.0, tk.END)
        if summary and summary.get("success"):
            self.tier2_summary_text.insert(
                tk.END,
                json.dumps(summary.get("data", {}), indent=2, ensure_ascii=False),
            )
        else:
    ...
    def show_hdfs_status(self):
        ...
        def _show():
            try:
                ...
                messagebox.showinfo(
                    "HDFS 상태", status.get("report", "상태를 가져올 수 없습니다.")
                )
            except Exception as e:
    ...
    def start_spider(self):
        ...
        def _start():
            try:
                ...
                self.control_log_text.insert(
                    tk.END, f"[{datetime.now()}] Spider 시작: {spider} @ {host}\n"
                )
                self.control_log_text.insert(
                    tk.END,
                    f"결과: {json.dumps(result, indent=2, ensure_ascii=False)}\n\n",
                )
                self.control_log_text.see(tk.END)
            except Exception as e:
                self.control_log_text.insert(
                    tk.END, f"[{datetime.now()}] 오류: {str(e)}\n\n"
                )
    ...
    def stop_spider(self):
        ...
        def _stop():
            try:
                ...
                self.control_log_text.insert(
                    tk.END, f"[{datetime.now()}] Spider 중지: {spider} @ {host}\n"
                )
                self.control_log_text.insert(
                    tk.END,
                    f"결과: {json.dumps(result, indent=2, ensure_ascii=False)}\n\n",
    ...
    def restart_pipeline(self):
        ...
        def _restart():
            try:
                ...
                self.control_log_text.insert(
                    tk.END, f"[{datetime.now()}] 파이프라인 재시작: {host}\n"
                )
                self.control_log_text.insert(
                    tk.END,
                    f"결과: {json.dumps(result, indent=2, ensure_ascii=False)}\n\n",
