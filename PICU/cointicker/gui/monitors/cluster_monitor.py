...
class ClusterMonitor:
    ...
    def _get_ssh_client(self, host: str, user: str, port: int = 22) -> Optional[paramiko.SSHClient]:
        """
        SSH 클라이언트 가져오기 (재사용)
    ...
    def execute_command(self, host: str, command: str) -> Dict[str, any]:
        ...
        if host == master.get('ip') or host == master.get('hostname'):
            ...
            worker = next((w for w in workers if w.get('ip') == host or w.get('hostname') == host), None)
            if not worker:
    ...
    def _fetch_node_status(self, host: str) -> Dict[str, any]:
        ...
        if ping_result.get("success"):
            ...
            # CPU 사용률
            cpu_result = self.execute_command(host, "top -bn1 | grep 'Cpu(s)' | awk '{print $2}' | cut -d'%' -f1")
            ...
            # 메모리 사용률
            mem_result = self.execute_command(host, "free | grep Mem | awk '{printf \"%.1f\", $3/$2 * 100.0}'")
            ...
            # 디스크 사용률
            disk_result = self.execute_command(host, "df -h / | tail -1 | awk '{print $5}' | sed 's/%//'")
            ...
            master = config.get('master', {})
            if host == master.get('ip') or host == master.get('hostname'):
                hadoop_result = self.execute_command(host, "jps | grep -i namenode || echo 'not running'")
                status["hadoop_status"] = "running" if "namenode" in hadoop_result.get("output", "").lower() else "stopped"

            # Scrapy 프로세스 확인
            scrapy_result = self.execute_command(host, "ps aux | grep scrapy | grep -v grep | wc -l")
            if scrapy_result.get("success"):
                try:
                    count = int(scrapy_result["output"].strip())
                    status["scrapy_status"] = f"{count} processes" if count > 0 else "stopped"
                except:
    ...
    def start_spider(self, host: str, spider_name: str) -> Dict[str, any]:
        ...
        Returns:
            실행 결과
        """
        command = f"cd ~/cointicker/worker-nodes && scrapy crawl {spider_name}"
