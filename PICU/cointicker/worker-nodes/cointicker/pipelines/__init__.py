"""
Scrapy Pipelines
고급 버전 - HDFSUploadManager를 사용한 자동 재업로드 기능 포함
"""
...
log_file = str(cointicker_root / "logs" / "pipelines.log")
...
class HDFSPipeline:
    ...
    def open_spider(self, spider):
        ...
        try:
            ...
            logger.error(f"Failed to initialize HDFS client: {e}", exc_info=True)
            self.hdfs_client = None
            self.upload_manager = None
    ...
    def _save_batch(self, spider):
        ...
        try:
            ...
            logger.error(f"Error saving batch to HDFS: {e}", exc_info=True)


__all__ = ["ValidationPipeline", "DuplicatesPipeline", "HDFSPipeline", "KafkaPipeline"]
